...
spec.test('methods `handle` handles an error and returns error codes', async (ctx) => {
...
spec.test('methods `handle` can fail fast', async (ctx) => {
  const handler = new Handler({
    failFast: true,
  });
  const recipes = [
...
spec.test('methods `handle` supports conditional handlers', async (ctx) => {
  const handler = new Handler();
  const recipes = [
...
spec.test('methods `handle` passes context to each handler', async (ctx) => {
  const handler = new Handler({
    ctx: { foo: 'foo' },
  });
  const recipes = [
...
spec.test('methods `handle` passes context to each condition', async (ctx) => {
  const handler = new Handler({
    ctx: { foo: 'foo' },
  });
  const recipes = [
...
spec.test('methods `handle` passes recipe to each handler', async (ctx) => {
  const handler = new Handler();
  const recipes = [
...
spec.test('methods `handle` passes recipe to each condition', async (ctx) => {
  const handler = new Handler();
  const recipes = [
